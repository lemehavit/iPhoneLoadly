...
| Apple login, 2FA, developer resources, provisioning and signing | `isideload` behind an internal `SigningProvider` adapter | Medium, hardware/account-gated | Use a dedicated Apple Account; request 2FA interactively; keep passwords in memory by default and persist encrypted credentials only after explicit opt-in. |
